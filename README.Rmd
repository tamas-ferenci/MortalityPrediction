...
```{r germanpuzzle, fig.cap = "A linear trend and a spline fitted on the 2015--2019 German mortality data and extrapolated to 2020 and 2021."}
...
The full source code allowing for complete reproducibility is openly available at: [https://github.com/tamas-ferenci/MortalityPrediction](https://github.com/tamas-ferenci/MortalityPrediction).
...
```{r errorWHOAI, fig.cap = "Different error metrics (logMSE, MAPE, and Bias) for all methods and all possible parameter combinations of all methods. Parameters of the scenario are set to the base case values.", fig.height = 12}
pd <- melt(rbind(
...
```{r errorscenarios, fig.cap = "Mean squared errors of the investigated methods by starting year on a logarithmic scale (with k = 3 for the WHO method and tkpy = 1/7 for the AI approach) for the four defined scenarios."}
